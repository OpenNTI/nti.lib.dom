...
function scrollParent(node) {
	...
	return (
		parents(node.parentNode).find(p => canScroll(p)) ||
		document.scrollingElement ||
		document.documentElement
	);
}

const shouldBeWindow = el =>
	!el ||
	(ROOT_ELEMENTS.test(el.tagName) && el.clientHeight <= el.scrollHeight);

export const getScrollParent = el =>
	(x => (shouldBeWindow(x) ? WINDOW : x))(scrollParent(el));
